...
def get_url(uri=None, full_address=False):
	...
	if not (frappe.conf.restart_supervisor_on_update or frappe.conf.restart_systemd_on_update or frappe.conf.proxied) and host_name and not url_contains_port(host_name) and port:
...
def md_to_html(markdown_text):
	...
	html = None
	try:
		html = UnicodeWithAttrs(_markdown(markdown_text or '', extras=extras, footnote_title="Jump back to footnote %d in the text.", footnote_return_symbol='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16"><path fill-rule="evenodd" d="M3.47 7.78a.75.75 0 010-1.06l4.25-4.25a.75.75 0 011.06 0l4.25 4.25a.75.75 0 01-1.06 1.06L9 4.81v7.44a.75.75 0 01-1.5 0V4.81L4.53 7.78a.75.75 0 01-1.06 0z"></path></svg>'))
